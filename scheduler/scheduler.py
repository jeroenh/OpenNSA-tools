...
SCHEDULE = [
            (("aist.ets","ps-83"),     ("max.ets","ps-83"),     "aist.ets"),
    ...
    def doTerminate(self, provider_nsa, connection_id):
        if connection_id:
            ...
            try:
                ...
                print "TerminationError: %s" % e 
                self.logger.info("Terminating %s" % self.reservationDescr)
                self.logger.info("TerminateError: %s" % e)
        else:
